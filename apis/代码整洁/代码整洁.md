...
### 使用 Object.assign 设置默认对象，现在可以使用 ... 展开操作符进行对象属性合并
...
并且更容易进行测试， 当你可以使用函数式编程风格时请尽情使用。参看《函数式编程》
...
你应该考虑使用 TypeScript 。 保持你的 JavaScript 简洁， 编写良好的测试， 并有良好的代码审阅。
...
### 让对象拥有私有成员
使对象拥有私有成员，OOP思路，js一直支持得不好，这个可以通过闭包来实现（针对 ES5 或更低），或TS或node12以上
...
An "is-a" relationship and not a "has-a" relationship
...
### 单一职责原则 (SRP)
 ...
 “永远不要有超过一个理由来修改一个类”。 给一个类塞满许多功能， 就像你在航班上只能带一个行李箱一样， 这样做的问题你的类不会有理想的内聚性， 将会有太多的理由来对它进行修改。
...
注释是代码的辩解， 不是要求。 多数情况下， 好的代码就是文档（高段位，代码即文档）。
